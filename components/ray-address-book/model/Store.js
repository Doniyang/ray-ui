...
export default class Store {
	...
	get datasplitor(){
		return this.options.itemSplitor || 'PY'
	}
	/**
	 * 树结构数组转换数组
	 ...
	 */
	getAddrList(pid){
		 let nodeList = this.getDataList(pid)
		 let splitor = this.datasplitor
	     return Array.from(new Set(nodeList.map(item=>item[splitor]))).sort().map(letter=>({code:letter,children:  nodeList.filter(d=>d[splitor] ===letter)}))	
	}
	/**
	 * 获取列表数据
	 * @param {Object} pid
